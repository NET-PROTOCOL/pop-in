...
//추가
int L2_msg_checkIfBroadcast(uint8_t* msg)
{
    return (msg[L2_MSG_OFFSET_TYPE] == L2_MSG_TYPE_BROADCAST);
}

int L2_msg_checkIfPresence(uint8_t* msg)
{
    return (msg[L2_MSG_OFFSET_TYPE] == L2_MSG_TYPE_PRESENCE);
}

uint8_t L2_msg_encodeBroadcast(uint8_t* msg_data, uint8_t* data, int seq, int len)
{
    msg_data[L2_MSG_OFFSET_TYPE] = L2_MSG_TYPE_BROADCAST;
    msg_data[L2_MSG_OFFSET_SEQ] = seq;
    memcpy(&msg_data[L2_MSG_OFFSET_DATA], data, len*sizeof(uint8_t));
