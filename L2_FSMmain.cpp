...
#define MAX_SESSIONS 32  // 동시 세션 최대 수
typedef struct {
    uint8_t userId[USER_ID_SIZE];
    uint8_t status;      // 0: 비활성, 1: 액티브, 2: 대기 중
    uint8_t seqNum;      // 이 세션의 현재 시퀀스 번호
    // 기타 세션 관련 정보
} L2_SessionInfo;

static L2_SessionInfo sessionList[MAX_SESSIONS];
...
// 세션 관리 함수들
int L2_createSession(uint8_t* userId) {
    // 새 세션 생성
}

int L2_findSession(uint8_t* userId) {
    // userId에 해당하는 세션 찾기
}

void L2_updateSessionStatus(uint8_t* userId, uint8_t newStatus) {
    // 세션 상태 업데이트
}

void L2_removeSession(uint8_t* userId) {
    // 세션 제거
}
    ...
    switch (main_state)
    {
        case L2STATE_IDLE: //IDLE state description
            ...
            else if (L2_event_checkEventFlag(L2_event_dataRcvd)) //if data reception event happens
            {
                ...
                if (L2_msg_checkIfBroadcast(dataPtr))
                {
                // 브로드캐스트 메시지 처리
                    L3_LLI_broadcastInd(L2_msg_getWord(dataPtr), srcId, size-L2_MSG_OFFSET_DATA);
                }
#ifndef DISABLE_ARQ                
