...
// L2_LLinterface.cpp에 추가
static uint8_t presenceDetectionActive = 0;
static uint8_t nearbyBaseStations[MAX_BASE_STATIONS];
static uint8_t nearbyBaseStationCount = 0;

void L2_LLI_startPresenceDetection(void)
{
    presenceDetectionActive = 1;
    // 하위 레이어에 신호 감지 시작 요청
    phymac_startScanning();
}

void L2_LLI_stopPresenceDetection(void)
{
    presenceDetectionActive = 0;
    // 하위 레이어에 신호 감지 중지 요청
    phymac_stopScanning();
}
...
{
    for(int i = 0; i < nearbyBaseStationCount; i++) {
        if (nearbyBaseStations[i] == baseStationId) {
            return 1;
        }
    }
    return 0;
...
// 하위 레이어에서 호출되는 콜백 함수
void L2_LLI_baseScanIndFunc(uint8_t* baseList, uint8_t count)
{
    // 감지된 기지국 목록 저장
    memcpy(nearbyBaseStations, baseList, count);
    nearbyBaseStationCount = count;

    // 상위 계층에 알림
    L3_LLI_baseStationScanInd(baseList, count);
}

void L2_LLI_broadcastData(uint8_t* msg, uint8_t size)
{
    // 브로드캐스트 주소(255)로 데이터 전송
    phymac_dataReq(msg, size, 255);
    txType = msg[L2_MSG_OFFSET_TYPE];
}